    ...
    match node {
      ...
      Node::ConditionalExpression{name, children} => {
        match(self.run(&children[0]), self.run(&children[1])){
            ...
            _ => Err("Invalid Conditional Expression")
        }
      },
      ...
      Node::Expression{children} => { //boolean, function_call, number, string, identifier, math_expression
        match children[0] {
          Node::MathExpression{..} | Node::Number{..} | Node::FunctionCall{..} | Node::String{..} | Node::Bool{..} | Node::Identifier{..} | Node::ConditionalExpression{..}=> {
            self.run(&children[0])
          },
