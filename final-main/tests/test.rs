...
use asalang::{program, Node, Value, run};
use nom::IResult;
...
//My Tests
test!(my_test_1, r#"3 ^ 5"#, Ok(Value::Number(243)));
test!(my_test_2, r#"let this = 9;"#, Ok(Value::Number(9)));
test!(my_test_3, r#"(3 ^ 5)/27"#, Ok(Value::Number(9)));
test!(my_test_4, r#"something(1,2,3)"#, Err("Undefined function"));
test!(my_test_5, r#"let me = "cse262";"#, Ok(Value::String("cse262".to_string())));
test!(conditional1, r#"1 + 2 == 3;"#, Ok(Value::Bool(true)));
test!(conditional2, r#"2^4 == 16"#, Ok(Value::Bool(true)));
test!(conditional3, r#"17 - 5 == 11 * 2"#, Ok(Value::Bool(false)));
